...
export function BlockCompleteCard({ trackLabel }: { trackLabel: string }) {
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 md:p-8">
      <h3 className="text-2xl font-semibold text-white mb-3">🎉 Блок 3 завершён</h3>
...
export const block3KnowledgeQuestions = [
  { q: "В чём главное преимущество мультиагентной системы над одним агентом?", options: ["Дешевле", "Параллельная работа + специализация + контроль качества", "Проще настроить", "Работает без промптов"], correct: 1 },
  { q: "Какую роль выполняет Orchestrator?", options: ["Пишет контент", "Управляет другими агентами и разбивает задачи", "Мониторит бюджет токенов", "Хранит данные"], correct: 1 },
  { q: "Что такое QA-агент?", options: ["Агент для квизов", "Агент контроля качества — проверяет результат до отправки", "Агент для найма", "Агент квалификации лидов"], correct: 1 },
  { q: "Для чего нужен принцип изоляции агентов?", options: ["Для экономии токенов", "Чтобы агент не знал лишнего и не мог навредить системе", "Для скорости работы", "Для красивого кода"], correct: 1 },
  { q: "Что происходит при превышении бюджета токенов?", options: ["Агент молчит", "Watchdog останавливает агента и отправляет алерт", "Система перезагружается", "Токены берутся из следующего месяца"], correct: 1 },
];
...
const quizOptions: QuizOption[] = [
  {
    id: "freelancer",
    label: "🧑‍💼 Работу с клиентами и задачами — КП, трекинг, переписка",
    nextLesson: 8,
  },
  {
    id: "business",
    label: "🏢 Продажи, поддержку и отчёты — CRM, клиенты, аналитика",
    nextLesson: 10,
  },
  {
    id: "content",
    label: "📢 Создание и публикацию контента — Research → Write → Publish",
    nextLesson: 12,
  },
  {
    id: "student",
...
export function QuizBlock({
  ...
  function handleSelect(option: QuizOption) {
    setTrack(option.id);
    localStorage.setItem("block3_track", option.id);
    ...
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 md:p-8">
        ...
        <p className="text-zinc-300 mb-4">
          Следующий урок:{" "}
          <a
            href={`/dashboard/course/block/3/lesson/${selected?.nextLesson}`}
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 md:p-8">
      ...
      <p className="text-zinc-400 mb-4">
        Выбери свой трек — дальше учишься на конкретных схемах для своей задачи.
      </p>
      <div className="flex flex-col gap-3">
