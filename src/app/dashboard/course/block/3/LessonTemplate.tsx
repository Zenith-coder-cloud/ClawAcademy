  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {variant.troubleshoot.length > 0 && (
        <TroubleshootSearch items={variant.troubleshoot} />
      )}
    </div>
  );
}
...
export default function LessonTemplate({ lessonId }: { lessonId: number }) {
  ...
  const handleNextLesson = useCallback((href: string) => {
    if (lesson) {
      localStorage.setItem(`b3_lesson_${lesson.id}_done`, "1");
    }
        ...
        <aside className="hidden md:block w-56 shrink-0">
          <div className="sticky top-6 bg-zinc-900 border border-zinc-800 rounded-xl p-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-3 gap-1 mb-3">
              ...
              <Link href="/dashboard/course/block/3/lesson/1" className="text-center text-xs py-1.5 rounded-lg border border-[#FF4422] text-[#FF4422]">
                Б 3
              </Link>
