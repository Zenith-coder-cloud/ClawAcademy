"use client";

import Image from "next/image";
import { useState } from "react";

interface Section {
  title: string;
  topics: string[];
}

interface Block {
  num: number;
  locked: boolean;
  title: string;
  subtitle: string;
  desc: string;
  topics?: string[];
  sections?: Section[];
  tier?: string;
}

const blocks: Block[] = [
  {
    num: 0,
    locked: false,
    title: "Вход — что такое ИИ-агент",
    subtitle: "«Что такое ИИ-агент и почему это меняет всё»",
    desc: "Ты узнаешь чем агент отличается от обычного чат-бота, почему OpenClaw — это не просто \"ещё один ChatGPT\", и увидишь реальные кейсы людей которые уже изменили свою жизнь с помощью агентов — в бизнесе, творчестве, учёбе и повседневной жизни.",
    topics: [
      "Что такое ИИ-агент и как он думает и действует",
      "OpenClaw vs ChatGPT vs n8n — в чём принципиальная разница",
      "Реальные кейсы: предприниматель автоматизировал команду, дизайнер делегировал рутину, студент выстроил систему обучения, фрилансер построил доход",
      "Твой результат и карта пути: от Block 0 до Block 5",
    ],
  },
  {
    num: 1,
    locked: true,
    tier: "Genesis",
    title: "Установка и первый агент",
    subtitle: "«Первый агент за 15 минут»",
    desc: "Никакой теории — только практика. Ставим OpenClaw, запускаем первого агента и сразу видим результат. Неважно кто ты — к концу блока у тебя будет живой работающий агент.",
    topics: [
      "Установка OpenClaw: Mac, Linux, Windows, VPS — пошагово",
      "Первый запуск и базовая настройка под твои задачи",
      "Создаём Telegram агента с нуля за 15 минут",
      "Подключение к внешним сервисам: Google, Notion, Calendar",
      "Первая автоматизация: Бизнес — агент отвечает клиентам / Творчество — агент ведёт соцсети / Учёба — агент-напоминалка / Заработок — агент-ресёрчер / Жизнь — агент облегчает рутину",
    ],
  },
  {
    num: 2,
    locked: true,
    tier: "Genesis",
    title: "Агент на работе: первые реальные кейсы",
    subtitle: "«От настройки к результату за 30 минут»",
    desc: "Заставляем агента работать на тебя. 5 уроков с готовыми промптами и реальными кейсами — выбираешь свой трек и получаешь результат уже в первый день.",
    topics: [
      "Агент-автоответчик: отвечает за тебя пока ты занят — экономишь 2 часа в день",
      "Агент-ресёрчер: аналитические отчёты по любой теме за 5 минут вместо 2 часов",
      "Email-агент: классифицирует inbox, готовит черновики — для фрилансеров",
      "Контент-агент: план на неделю и готовые посты для TG/Instagram — для блогеров",
      "Агент-парсер рынка: мониторинг конкурентов и цен автоматически — для бизнеса",
    ],
  },
  {
    num: 3,
    locked: true,
    title: "Мультиагент и автоматизация",
    subtitle: "«Перестань управлять агентами — пусть они управляют собой»",
    desc: "34 урока, 10 треков. Строим системы где несколько агентов работают в команде — от архитектуры и ролей до запуска автономной системы по расписанию. Треки: Фрилансер, Бизнес, Контент, Заработок, Студент, Инвестор, Продавец, Разработчик, Маркетолог, HR, Для жизни.",
    sections: [
      {
        title: "Почему мультиагент (уроки 1–3)",
        topics: [
          "Один агент — это потолок: где физические ограничения",
          "Роли: Orchestrator, Workers, Monitor — кто есть кто",
          "Квиз: выбери свой трек — 10 направлений под любую задачу",
        ],
      },
      {
        title: "Архитектура команды (уроки 4–7)",
        topics: [
          "Делегирование и изоляция: чёткий Input/Output для каждого агента",
          "Cron + мультиагент: система работает без тебя по расписанию",
          "QA-агент: контроль качества до того как результат дойдёт до тебя",
          "Живой пример: система Zen + Dev + Mark в действии",
        ],
      },
      {
        title: "Практика по трекам (уроки 8–34)",
        topics: [
          "Фрилансер: КП + трекер + коммуникатор + биллинг",
          "Бизнес: CRM + поддержка + аналитика + воронка продаж",
          "Контент: Research → Write → Edit → Publish → Distribute",
          "Заработок: мониторинг рынков + планировщик возможностей",
          "Студент: система обучения + подготовка к экзаменам",
          "Инвестор: мониторинг рынков + анализ проектов",
          "Продавец: описания товаров + мониторинг конкурентов",
          "Разработчик: code review + документация и онбординг",
          "Маркетолог: аналитика и отчёты + A/B тесты",
          "HR / Рекрутер: скрининг кандидатов + онбординг",
          "Для жизни: планирование дня + финансы и привычки",
        ],
      },
      {
        title: "Управление и мониторинг (уроки 16–17)",
        topics: [
          "Токены, бюджеты, watchdog — контроль расходов и зацикливания",
          "Обработка ошибок и fallback — система которая умеет восстанавливаться",
        ],
      },
      {
        title: "Безопасность и финал (уроки 18–20)",
        topics: [
          "Границы агента: принцип минимальных прав",
          "Секреты и API ключи: безопасное хранение и ротация",
          "Капстоун: запусти свою первую мультиагентную систему",
        ],
      },
    ],
  },
  {
    num: 4,
    locked: true,
    title: "Продай своего агента: ClawHub и виртуальный сотрудник",
    subtitle: "«Ты строил — теперь продаёшь. Первые $1000 с агентов»",
    desc: "30 уроков, 10 треков монетизации. Перестаёшь быть пользователем и становишься поставщиком: виртуальный сотрудник, ClawHub, SaaS, бизнес, контент, студент, инвестор, продавец, разработчик, маркетолог, HR, для жизни.",
    sections: [
      {
        title: "Разворот и базовые пути (уроки 1–12)",
        topics: [
          "Твоя система стала продуктом",
          "Выбери свой путь: 10 треков монетизации",
          "Фрилансер: агент-сотрудник, white-label, SLA, продакшн",
          "ClawHub: маркетплейс, упаковка, первая продажа",
          "SaaS: multi-tenant, биллинг, капстоун",
        ],
      },
      {
        title: "Бизнес и контент (уроки 13–16)",
        topics: [
          "Бизнес: агентское агентство — системы под ключ для других бизнесов",
          "Бизнес: первый контракт на автоматизацию",
          "Контент: агент-медиа — продаём контент-системы",
          "Контент: первый медиа-клиент и масштаб",
        ],
      },
      {
        title: "Студент, инвестор, продавец (уроки 17–22)",
        topics: [
          "Студент: образовательный агент как продукт",
          "Студент: подписка и масштаб через сообщества",
          "Инвестор: финансовый аналитик как сервис",
          "Инвестор: масштаб — от канала к аналитическому сервису",
          "Продавец: е-ком агент как сервис для магазинов",
          "Продавец: масштаб — от одного клиента к агентству",
        ],
      },
      {
        title: "Разработчик, маркетолог, HR, жизнь (уроки 23–30)",
        topics: [
          "Разработчик: агент как API — продаём техническим командам",
          "Разработчик: developer tool экосистема и рост",
          "Маркетолог: агентство на агентах",
          "Маркетолог: white-label маркетинг для малого бизнеса",
          "HR: рекрутинговое агентство на агентах",
          "HR: масштаб — от фриланса к рекрутинговому агентству",
          "Для жизни: агенты повседневности как продукт",
          "Для жизни: масштаб — от бота к экосистеме",
        ],
      },
    ],
  },
  {
    num: 5,
    locked: true,
    title: "Агентский бизнес",
    subtitle: "«От $1000 к $5000+»",
    desc: "26 уроков, 10 треков. Строишь систему которая работает без тебя.",
    sections: [
      {
        title: "Основы агентского бизнеса (уроки 1–4)",
        topics: [
          "От первого дохода к системе — разница между фрилансом и бизнесом",
          "Первый помощник: делегирование без потери контроля",
          "Машина лидогенерации: клиенты приходят сами",
          "Личный бренд ИИ-эксперта: позиционирование которое продаёт",
        ],
      },
      {
        title: "Масштаб по треку (уроки 5–24)",
        topics: [
          "Фрилансер: от проектов к ретейнерам + субподряд и партнёрства",
          "Бизнес: агентство внутри компании + операционная система",
          "Контент: медиа-империя + монетизация портфолио",
          "Студент: стартап + привлечение инвестиций",
          "Инвестор: фонд на агентах + масштаб и трек-рекорд",
          "Продавец: маркетплейс агентство + собственный e-com бренд",
          "Разработчик: SaaS на агентах + рост и enterprise",
          "Маркетолог: performance + стратегическое консалтинговое агентство",
          "HR: кадровое агентство + HR-аутсорсинг как SaaS",
          "Для жизни: сообщество + личный коучинг с агентами",
        ],
      },
      {
        title: "Финмодель и капстоун (уроки 25–26)",
        topics: [
          "Финансовая модель агентства: юнит-экономика, LTV, CAC, сценарии",
          "Капстоун: агентство запущено — аудит, метрики, 90-дневный план",
        ],
      },
    ],
  },
];

function BlockCard({ block }: { block: Block }) {
  const [expanded, setExpanded] = useState(false);

  return (
    <div className="bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800">
      <Image
        src={`/covers/block${block.num}.png`}
        alt={block.title}
        width={800}
        height={400}
        className="w-full h-64 object-cover"
      />
      <div className="p-6">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="bg-zinc-800 text-zinc-400 text-xs px-3 py-1 rounded-full">
            Block {block.num}
          </span>
          {block.locked && (
            <span className="bg-[#FF4422]/10 text-[#FF4422] text-xs px-3 py-1 rounded-full">
              🔒 {block.tier ?? "Pro / Elite"}
            </span>
          )}
        </div>
        <h3 className="text-white font-bold text-xl mt-3">{block.title}</h3>
        <p className="text-zinc-500 text-sm italic mt-1">{block.subtitle}</p>
        <p className="text-zinc-400 text-sm mt-4 leading-relaxed">{block.desc}</p>

        <button
          onClick={() => setExpanded(!expanded)}
          className="mt-4 text-[#FF4422] text-sm font-semibold hover:underline focus:outline-none"
        >
          {expanded ? "Скрыть ▲" : "Подробнее ▼"}
        </button>

        {expanded && (
          <>
            {block.topics && (
              <ul className="mt-4 space-y-2">
                {block.topics.map((t, i) => (
                  <li key={i} className="text-zinc-400 text-sm flex gap-2">
                    <span className="text-[#FF4422] mt-1">•</span>
                    <span>{t}</span>
                  </li>
                ))}
              </ul>
            )}
            {block.sections &&
              block.sections.map((sec, si) => (
                <div key={si}>
                  <h4 className="text-zinc-300 font-semibold text-sm mt-4 mb-2">
                    {sec.title}
                  </h4>
                  <ul className="space-y-2">
                    {sec.topics.map((t, ti) => (
                      <li key={ti} className="text-zinc-400 text-sm flex gap-2">
                        <span className="text-[#FF4422] mt-1">•</span>
                        <span>{t}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ))}
          </>
        )}
      </div>
    </div>
  );
}

export default function CourseProgram() {
  return (
    <section className="max-w-6xl mx-auto px-4 py-20">
      <h2 className="text-3xl font-bold text-white text-center mb-12">
        Программа курса
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {blocks.map((block) => (
          <BlockCard key={block.num} block={block} />
        ))}
      </div>
    </section>
  );
}
